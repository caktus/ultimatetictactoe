...
Tictactoe is deployed on the following stack.
...
As documented you should create/import your Celery app in ``tictactoe/__init__.py`` so that you
can run the worker via::

    celery -A tictactoe worker

Additionally you will need to configure the project settings for Celery::

    # tictactoe.settings.staging.py
    import os
...
You will also need to add the ``BROKER_URL`` to the ``tictactoe.settings.production`` so
...
RabbitMQ user will be named tictactoe_<environment> and the vhost will be named tictactoe_<environment>
